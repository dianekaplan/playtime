 ...
 * check valid vs invalid, and report the verdict
 * */
...
long long GetCard()
{
    ...
    // Card length should be 13-16 digits,
    // so I started with a do/while loop
    // Commenting that out since 'check' wants to see 
    // 1234567890 go through and get evaluated as INVALID
    // do
    // {
    printf("Number: ");
 ...
 * Multiply every other digit by 2, starting with 
...
{
    // I got the idea to go digit by digit via modulus and dividing by 10 
    // from stack overflow (from searching how to iterate through the digits of a long long)
    ...
    while ( card > 0 )
    {
        ...
        { 
            ...
            // doubling it gives us a value between 10 - 18, so we need to add 1 + the last digit
            else 
            {
